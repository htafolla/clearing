...
`GET /agents.md`, `GET /llms.txt`, `GET /.well-known/agent.json`, `GET /.well-known/agent-tools-verify.txt`, `GET /v1/listed` are public and unpaid. Pay pin ($0.01) + live HTTPS MCP or hangar store → listed. Identity-only cards are not listed. No extra directory fee.
