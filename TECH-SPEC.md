...
`GET /agents.md`, `GET /llms.txt`, `GET /.well-known/agent.json`, `GET /.well-known/x402`, `GET /.well-known/agent-tools-verify.txt`, `GET /v1/listed`, `GET /v1/online`, `GET /v1/blip/owned`, `GET /v1/blip/metadata/:id` are public and unpaid. Hangar skill `blip` (`GET|POST /v1/blip`) is x402-metered: quote before work, ZigZag-shaped EIP-3009 (`signer:zigzag`, not marketplace), LOCKED escalator `price¢ = max(5, round(5 + 550 * (mintIndex/555)**2))`, then factory plant `blip` + Base Blips ERC-721 to the payer. mintIndex = settled paid count / collection totalSupply. See `docs/BLIPS.md`. `/.well-known/x402` is JSON with `agentToolsVerify` (default `public/.well-known/x402`, override `CLEARING_AGENT_TOOLS_VERIFY` / `AGENT_TOOLS_VERIFY_DESCRIPTOR`). The verify.txt file is the rippel.ai ATC claim and must stay `atc_ahATpKU6I8yhcD0aIdaKZp3ONf0bjyj9`. Pay pin ($0.01) + Groover (DID/GRVR) + Dynamo solar (PASS or citation) + live HTTPS MCP or hangar store + online (health ok within 15 minutes, `probeIntervalMs`) → listed. Identity-only cards are not listed. Pin alone is not enough. No extra directory fee.
