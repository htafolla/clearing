...
export function isExtractPath(pathname: string): boolean {
  return (
    ...
    pathname.startsWith('/v1/listings') ||
    pathname === '/agents.md' ||
    pathname === '/llms.txt' ||
...
export async function handleHangar(req: Request, ctx: ClearingContext): Promise<Response> {
  ...
  if (ping) return ping;
  const listings = await handleListings(req, ctx);
  ...
  const skim = await handleSkim(req, ctx);
  if (skim) return skim;
  const blip = await handleBlip(req, ctx);
  if (blip) return blip;
  const card = await handleCard(req, ctx);
  if (card) return card;
